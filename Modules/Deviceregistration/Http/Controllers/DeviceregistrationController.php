...
namespace Modules\Deviceregistration\Http\Controllers;

use Illuminate\Http\Request;
use Modules\Deviceregistration\Entities\Deviceregistration;
use Modules\Base\Http\Controllers\BaseController;
use Modules\Deviceregistration\Http\Requests\DeviceregistrationFormRequest;
        ...
        if(permission('deviceregistration-access')){
            if($request->ajax()){
                ...
                if (!empty($request->deviceId)) {
                    $this->model->setDeviceId($request->deviceId);
                }
                ...
                foreach ($list as $value) {
                    ...
                    if(permission('deviceregistration-edit')){
                        $action .= ' <a class="dropdown-item edit_data" data-id="' . $value->id . '"><i class="fas fa-edit text-primary"></i> Edit</a>';
                    }
                    if(permission('deviceregistration-delete')){
                    ...
                    $row[] = $value->name;
                    $row[] = $value->deviceId;
                    $row[] = permission('deviceregistration-edit') ? change_status($value->id,$value->status,$value->name) : STATUS_LABEL[$value->status];;
