...
@include('refdepartment::view-modal')
@include('refdepartment::add-edit-modal')
@endsection
...
$(document).ready(function(){
    ...
    table = $('#dataTable').DataTable({
        ...
        "ajax": {
            "url": "{{route('refdepartment.datatable.data')}}",
            "type": "POST",
            "data": function (data) {
        ...
        "buttons": [
            @if (permission('refdepartment-report'))
            {
    ...
    $(document).on('click', '#save-btn', function () {
        ...
        let url = "{{route('refdepartment.store.or.update')}}";
        let id = $('#RefDepartmentId').val();
        let method;
        if (id) {
            method = 'update';
        } else {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#view_modal .modal-title').html(
                        '<i class="fas fa-eye"></i> <span>RefDepartment</span>');
                },
    ...
    $(document).on('click', '.delete_data', function () {
        let RefDepartmentId    = $(this).data('id');
        // let name  = $(this).data('name');
        let name  = "RefDepartment";
        console.log(name);
        let row   = table.row($(this).parent('tr'));
        let url   = "{{ route('refdepartment.delete') }}";
        let response = delete_data(RefDepartmentId, url, table, row, name);
    ...
    function multi_delete(){
        ...
        if(ids.length == 0){
            ...
            let url = "{{route('refdepartment.bulk.delete')}}";
    ...
    $(document).on('click', '.change_status', function () {
        ...
        let name  = $(this).data('name');
        let row   = table.row($(this).parent('tr'));
        let url   = "{{ route('refdepartment.change.status') }}";
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: url,
                    type: "POST",
                    data: { RefDepartmentId: RefDepartmentId,Status:Status, _token: _token},
                    dataType: "JSON",
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#store_or_update_modal .modal-title').html(
                    '<i class="fas fa-edit"></i> <span>Edit RefDepartment</span>');
