...
@include('refsocialbehavior::view-modal')
@include('refsocialbehavior::add-edit-modal')
@endsection
...
$(document).ready(function(){
    ...
    table = $('#dataTable').DataTable({
        ...
        "ajax": {
            "url": "{{route('refsocialbehavior.datatable.data')}}",
            "type": "POST",
            "data": function (data) {
        ...
        "buttons": [
            @if (permission('refsocialbehavior-report'))
            {
    ...
    $(document).on('click', '#save-btn', function () {
        ...
        let url = "{{route('refsocialbehavior.store.or.update')}}";
        let id = $('#SocialBehaviorId').val();
        let method;
        if (id) {
            method = 'update';
        } else {
    ...
    $(document).on('click', '.delete_data', function () {
        ...
        let url   = "{{ route('refsocialbehavior.delete') }}";
    ...
    function multi_delete(){
        ...
        if(ids.length == 0){
            ...
            let url = "{{route('refsocialbehavior.bulk.delete')}}";
...
$(document).on('click', '.change_status', function () {
    ...
    let name  = $(this).data('name');
    let row   = table.row($(this).parent('tr'));
    let url   = "{{ route('refsocialbehavior.change.status') }}";
    Swal.fire({
...
$(document).on('click', '.edit_data', function () {
    ...
    if (id) {
        $.ajax({
            url: "{{route('refsocialbehavior.edit')}}",
            type: "POST",
            ...
            success: function (data) {
                ...
                $('#SocialBehaviorCode').val(data.SocialBehaviorCode);
                $('#Description').val(data.Description);
                $('#SocialBehaviorId').val(data.SocialBehaviorId);
                ...
                $('#store_or_update_modal .modal-title').html(
                    '<i class="fas fa-edit"></i> <span>Edit RefSocialBehavior</span>');
