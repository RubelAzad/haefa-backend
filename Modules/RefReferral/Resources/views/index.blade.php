                    ...
                    <form id="form-filter">
                        <div class="row">
                            <div class="form-group col-md-4">
                                <label for="name">RCode</label>
                                <input type="text" class="form-control" name="name" id="name" placeholder="Enter RCode">
                            </div>
...
@include('refreferral::view-modal')
@include('refreferral::add-edit-modal')
@endsection
...
$(document).ready(function(){
    ...
    table = $('#dataTable').DataTable({
        ...
        "ajax": {
            "url": "{{route('refreferral.datatable.data')}}",
            "type": "POST",
            "data": function (data) {
        ...
        "buttons": [
            @if (permission('refreferral-report'))
            {
    ...
    $(document).on('click', '#save-btn', function () {
        ...
        let url = "{{route('refreferral.store.or.update')}}";
        let id = $('#RId').val();
        let method;
        if (id) {
            method = 'update';
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#view_modal .modal-title').html(
                        '<i class="fas fa-eye"></i> <span>refreferral</span>');
                },
    ...
    $(document).on('click', '.delete_data', function () {
        ...
        let name  = "refreferral";
        console.log(name);
        let row   = table.row($(this).parent('tr'));
        let url   = "{{ route('refreferral.delete') }}";
    ...
    function multi_delete(){
        ...
        if(ids.length == 0){
            ...
            let url = "{{route('refreferral.bulk.delete')}}";
...
$(document).on('click', '.change_status', function () {
    ...
    let name  = $(this).data('name');
    let row   = table.row($(this).parent('tr'));
    let url   = "{{ route('refreferral.change.status') }}";
    Swal.fire({
        ...
        if (result.value) {
            $.ajax({
                url: url,
                type: "POST",
                data: { RId: RId,Status:Status, _token: _token},
                dataType: "JSON",
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#store_or_update_modal .modal-title').html(
                    '<i class="fas fa-edit"></i> <span>Edit refreferral</span>');
